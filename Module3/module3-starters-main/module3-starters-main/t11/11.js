...
const closer =  dialog.querySelector('span');
function openDialog(pic) {
...
for (let pic = 0; pic < picArray.length; pic++) {
  ...
  h2.textContent = picArray[pic].title;
  ...
  figcaption.textContent = picArray[pic].caption;
  figure.appendChild(figcaption);
  article.appendChild(figure);
  let p = document.createElement('p');
  p.textContent = picArray[pic].description;
  article.appendChild(p);
  article.addEventListener('click', () => openDialog(picArray[pic]))
